...
app.get('/api/health', (_, res) => res.json({ ok:true, service:'promptify-backend', time: new Date().toISOString() }));
...
app.post('/api/generate', upload.fields([{ name:'logo', maxCount:1 }, { name:'bg', maxCount:1 }]), async (req, res) => {
  try {
    const body = {
      name: req.body.name,
    ...
    // If model didn't return a full HTML doc, wrap it safely
    ...
    // Inject color tokens & social links if missing (best-effort)
    html = html.replace('</head>', `<style>:root{--primary:${data.colors.primary};--accent:${data.colors.accent};--bg:${data.colors.bg};}</style></head>`);
    if (logoDataURL) html = html.replace('<body', f'<body data-logo="embedded"').replace(/src="[^"]*logo[^"]*"/i, f'src="{logoDataURL}"');
    if (bgDataURL) html = html.replace(/background-image:[^;]+;/i, f'background-image:url("{bgDataURL}");');
